...
*PROD01(C,R,U)   Original production for urban-rural consumption 2018 (1000tons)
...
*CRPROD0(C,R,U) Original production for urban-rural consumption 2018 (1000tons)
*LVPROD0(C,R,U) Original lvstck prod. for urban-rural consumption 2018 (1000tons LW)
**NNQue: to avoid using IDEN when sum-up CRPROD(F,R,U) & LVPROD(L,R,U) that have
**different sizes, we need to declare them with the same index C, i.e.
**CRPROD(C,R,U) & LVPROD(C,R,U), but the data table keep the same as before.
...
*NNQue2019: Karl added new parameter SHK for VPM2018
...
*NNQue2019: Testing
*PROD01(C,R,U)=SUM(F,IDEN1(C,F)*CRPROD0(F,R,U))+SUM(L,IDEN2(C,L)*LVPROD0(L,R,U));
*DISPLAY PROD01 ;
...
*NNQue2019: add parameter DFEEDPIG0 to calculate feed demand for pigs only
...
*NNQue: no need to use IDEN1 to calculate D0, since we declare DFEED0(C,R,U)
*instead of DFEED0(F,R,U)
...
TP(RW,RW)                        = 0     ;
*NNQue: in 2014 transportation cost by sea is about 200USD for 1 container of 25 tons
*from Hai Phong port to Sai Gon port (about 0.300-0.350 VND for 1Kg-Km by sea)
...
*FDLVEL(F,L)     = -LVFDEL(L,F)*(AVEPS0(L)/AVEPS0(F))*(SUMPROD0(L)/SUMPROD0(F));
FDLVEL(F,L)     = -LVFDEL(L,F)*(AVEPS0(L)/AVEPS0(F))
                 *(SUMPROD0(L)/(SUMPROD0(L)*MPAR(L,'FEEDCONV')*FEEDSHARE(L,F)));

DISPLAY AVEPS0,SUMPROD0,FDLVEL ;
*NNQue2019: in VPM2018 the formula to calculate FDLVEL has been revised to specify
*more accurate ratios between prices & quantities of diferent types of livestock
*products & crop outputs used for feed by livestock types. This
*make values of derived elasticity of feed demand wrt livestock prices more
*rational/reasonable.
*As SUMPROD0(F) appears to be Total Production of maize including for human
*consumption, and moreover it does not differentiate quantities of maize used
*for different types of pig mode. So instead of SUMPROD0(F) it is better to use
*SUMFEED0(F,L)=SUMPROD0(L)*MPAR(L,'FEEDCONV')*FEEDSHARE(L,F)
